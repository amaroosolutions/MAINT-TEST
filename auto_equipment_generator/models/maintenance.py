...
class MaintenanceAuto(models.Model):
    ...
    next_due_date = fields.Date(string="Next Due Date", compute="compute_next_due_date")
        ...
        for auto in auto_ids:
            ...
            if due_date and due_date >= current_date:
                ...
                if diff.days == auto.maintenance_name_id.new_request_generator_days:
                    ...
                    maintenance_id.onchange_maintenance_auto_id()
                    auto.last_date = auto.next_due_date
...
class MaintenanceRequest(models.Model):
    ...
    def action_create_po(self):
        po_ids = self.mapped('po_line_ids').mapped('state', 'not in', ['sent', 'draft'])
        if po_ids:
